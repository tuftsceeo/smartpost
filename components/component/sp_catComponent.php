 ...
 */
	...
	abstract class sp_catComponent{
		...
		/* Component menu with functions specific
		 * to the component.
		 */
		abstract public function componentMenu();
		...
		/**
		 * Add CSS common to all components
		 */ 
		static function enqueueBaseCSS(){
				wp_register_style( 'sp_catComponentCSS', plugins_url('/css/sp_catComponent.css', __FILE__));
				wp_enqueue_style( 'sp_catComponentCSS' );
		}
	 ...
	 static function enqueueBaseJS(){
			wp_register_script( 'sp_catComponentJS', plugins_url('/js/sp_catComponent.js', __FILE__));
		...
		function renderSettings(){
			...
			<div id="comp-<?php echo $this->ID ?>" data-compid="<?php echo $this->ID ?>" class="stuffbox sp_componentBox">
				...
				<div class="requiredAndDefault">
					...
					<input type="checkbox" class="compRestrictions" id="isDefault<?php echo '-' . $this->ID ?>" data-compid="<?php echo $this->ID ?>" name="isDefault<?php echo '-' . $this->ID ?>" value="1" <?php echo $this->isDefault ? 'checked="checked"' : '' ?> <?php echo $this->isRequired ? 'disabled="disabled"' : ''?> />
					<label for="isRequired<?php echo '-' . $this->ID ?>">Required</label>
					<input type="checkbox" class="compRestrictions" id="isRequired<?php echo '-' . $this->ID ?>" data-compid="<?php echo $this->ID ?>" name="isRequired<?php echo '-' . $this->ID ?>" value="1" <?php echo $this->isRequired ? 'checked="checked"' : '' ?> />
				</div>
				<img src="<?php echo IMAGE_PATH . '/rightArrow.png'; ?>" data-divID="comp-<?php echo $this->ID ?>-info" class="expandArrow" />
				<h2 class="componentTitle">
					<?php echo $this->name ?>
					<img src="<?php echo $this->getIcon() ?>" />
				</h2>
				<p><?php echo $this->description ?></p>
    <div id="comp-<?php echo $this->ID ?>-info" class="componentInfo">
					<form id="componentOptions-<?php echo $this->ID ?>-form">
					<?php $this->componentOptions() ?>
					<input type="hidden" id="compID" name="compID" value="<?php echo $this->ID?>" />
					<input type="hidden" id="typeID" name="typeID" value="<?php echo $this->typeID?>" />
					</form>
				</div><!-- end .componentInfo -->
			</div><!-- end comp-<?php echo $this->ID ?> -->
			<?php
		}
