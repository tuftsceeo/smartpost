...
if (!class_exists("sp_catContent")) {
	...
	class sp_catContent extends sp_catComponent{
		 ...
		 */
	 function install(){
	  self::installComponent('Content', 'Rich and plain text editor', __FILE__);
	 }
    ...
    <li class="stuffbox"><a href="#"><img src="<?php echo IMAGE_PATH . '/downArrow.png'; ?>" /></a>
		...
		function componentOptions(){
			...
			?>
					<p>Select content mode:</p>
					<table>
						<tr>
							<td>Plaintext Mode*</td>
							<td><input type="radio" name="content-mode-<?php echo $this->ID ?>" id="content-mode-<?php echo $this->ID ?>" value="plaintext" <?php echo $options->plaintext ? 'checked="checked"' : '' ?> class="content-comp" /></td>
						</tr>
						<tr>
							<td>Richtext Mode</td>
							<td><input type="radio" name="content-mode-<?php echo $this->ID ?>" id="content-mode-<?php echo $this->ID ?>" value="richtext" <?php echo $options->richtext ? 'checked="checked"' : '' ?> /></td>
						</tr>
					</table>
				<p>*Converting from Richtext to Plaintext will remove all styling tags</p>
			<?php		
		}
