...
var sp_postLink = {
 ...
 currUrl: "",
	    ...
	    switch(err) {
	      ...
	      default:
			...
			uploadStarted: function(i, file, len){
				...
				$(firstChild).replaceWith('<span id="uploadingLinkThumb-' + compID + '"><img src="' + IMAGE_PATH + '/loading.gif" /> Uploading image... </span>');
			},
	...
	rotateThumbs: function(compID){
		...
		if(results){ //we may not always find thumbnails
			...
			selectThumb.click(function(){
				if(currThumb.attr('data-thumbid') == 0){
					$('#thumbSelection-' + compID).html('Thumbnail saved!');
					$('#thumbSelection-' + compID).delay(3000).fadeOut();					
				}else{
					thisObj.saveThumb(currThumb, compID);
				}
			})
		}
	...
	saveLink: function(link, compID){
		var thisObj = this;
		var loading =	$('<p><img src="' + IMAGE_PATH + '/loading.gif" /> Loading link info</p>');
		$.ajax({
			url				   : ajaxurl,
			type      : 'POST',
			data			   : {action: 'saveLinkAJAX', nonce: spNonce, compID: compID, link: link},
			dataType  : 'html',
  ...
  var compID = $('#' + contentID).attr('data-compid');
		$.ajax({
			url				   : ajaxurl,
			type      : 'POST',
			data			   : {action: 'saveLinkDescAJAX', nonce: spNonce, compID: compID, desc: content},
			dataType  : 'json',
			success  : function(response, statusText, jqXHR){
 ...
 */
	...
	removeThumb: function(compID, thumbDiv){
		$.ajax({
				url				   : ajaxurl,
				type      : 'POST',
				data			   : {action: 'removeLinkThumbAJAX', nonce: spNonce, compID: compID},
				dataType  : 'json',
				success  : function(response, statusText, jqXHR){
					if(response.success){
						var replaceDiv = thumbDiv.find('a');
						if(replaceDiv.length == 0){
							replaceDiv = $('#thumb_results-' + compID);
						}
						replaceDiv.replaceWith('<p>Drag and drop your own thumbnail!</p>');
