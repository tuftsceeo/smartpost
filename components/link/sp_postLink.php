	...
	class sp_postLink extends sp_postComponent{
			...
			function renderViewMode(){
					$html .= '<div id="sp_link-' . $this->ID . '" class="sp_link">';
							$html .= '<a href="' . $this->url .'" target="_new">';
								$html .= !empty($this->url) ? $this->url : '';
							$html .= '</a>';
			...
			function renderThumb( $editMode = true ){
				$html = "";
				$img = wp_get_attachment_image($this->urlThumb, array(100, 100), false);
				if( $img ){
	     ...
	     $html .= '<input type="file" id="sp_link_browse-' . $this->ID . '" name="sp_link_browse-' . $this->ID . '" style="display: none" />';
			...
			function renderDesc( $editMode = true ){
				$placeholder = $editMode ? "Click to add a description" : '';
				
				$html .= '<div class="sp_link_right_wrapper">';
					
					$data     = $editMode ? 'data-compid="' . $this->ID .'"' : '';
					$editable = $editMode ? 'editable' : '';
					$linkClass = $editMode ? 'sp_link_desc' : 'sp_link_desc_view';
					
					$html .= '<div id="sp_link_desc-' . $this->ID . '" ' . $data . ' class="' . $linkClass . ' ' . $editable . '">';
						$html .= empty($this->urlDesc) ? $placeholder : $this->urlDesc;
					$html .= '</div>';
				
				if( count($this->urlThumbs) > 1 && $editMode){
					$html .= '<div id="thumbSelection-' . $this->ID . '" class="thumbSelection">';
						$html .= '<button type="button" id="prevThumb-' . $this->ID . '" class="sp_link_prev"></button>';
						$html .= '<button type="button" id="nextThumb-' . $this->ID . '" class="sp_link_next"></button>';
						$html .= '<small>Select a thumbnail</small>';
						$html .= '<button type="button" id="selectThumb-' . $this->ID . '" data-compid="' . $this->ID . '" class="sp_link_select_thumb">Ok</button>';
					$html .= '</div>';
				}
				
				$html .= '</div> <!-- end #sp_link_right_wrapper -->';
			...
			function setLink($url){
				...
				if(is_wp_error($success)){
						...
						//Set the url even if we get an error, should still
						//link to URL even if it fails test
						$this->url     = $url;
						$this->urlDesc = $url;
						$this->update(0);
						return $success;
			 ...
			 *
			 * @return bool|obj  True if succesfully parsed, otherwise a WP_Error object. 
			 */
			private function parseURL($url){
				$status = $this->is_url($url);
			...
			function downloadUrlThumb($thumbURL){
				  ...
				  wp_update_attachment_metadata( $attach_id, $attach_data );
						...
						error_log('getimagesize did not work');					
						return false;
					}			
			}
			 ...
			 * succesfully retrieved and http status of 200 verified, then url is valid
			 * 
