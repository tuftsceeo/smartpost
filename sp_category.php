...
if (!class_exists("sp_category")) {
	...
	class sp_category{
		...
		private $posts; //array of sp_post objects
		private $iconID;
		...
		function renderPostTree($post_status = 'publish', $post_parent = 0, $post_filters = null ){
			$args = array( 'numberposts' => -1, 'category' => $this->ID, 'post_status' => $post_status, 'post_parent' => $post_parent );
			$posts = get_posts( $args );
			
			$numOfPosts = count($posts);
			$catIcon    = wp_get_attachment_url($this->iconID, null, null, array('style' => 'vertical-align: text-bottom;'));			
		...
		function getComponentByName($name){
			...
			return new WP_Error ('notFound', __("Could not find a component by the name: " .
																								$name));
		}
